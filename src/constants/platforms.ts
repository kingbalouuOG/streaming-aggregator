/**
 * StreamingAggregator - Platform Configuration
 *
...
export const UK_PROVIDERS: Record<string, PlatformProvider> = {
  netflix: {
    id: 8,
    name: 'Netflix',
    color: '#E50914',
...
// Get provider by ID
export const getProviderById = (id: number): PlatformProvider | undefined => {
  return UK_PROVIDERS_ARRAY.find(provider => provider.id === id);
};

// ─────────────────────────────────────────────────────────────
// Rent/Buy to Subscription Mapping
// ─────────────────────────────────────────────────────────────
...
export const normalizePlatformName = (name: string | null | undefined): string | null | undefined => {
  if (!name) return name;
